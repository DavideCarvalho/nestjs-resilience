...
There's one channel per [`ResilienceEventType`](#events) under `aviary:resilience:<type>` — subscribe to each you care about (`circuit-opened`, `short-circuited`, `failover`, …).
